...
import { getBlogSingle, updateBlogWithId } from '../../common/apis/blogs';
...
const EditBlog = () => {
  ...
  const updateBlog = async () => {
    ...
    if (token) {
      dispatch(setLoading(true));
      const data = await updateBlogWithId(blog);
      if (data?.success) {
        setBlog(data?.data);
  ...
  useEffect(() => {
    getBlogs();
    if (blog?.createdBy?._id && userId !== blog.createdBy?._id) {
      navigate('/blogs');
    }
  ...
  return (
    <PageWithNavbar>
      {blog && <div className="flex flex-col bg-[#FAFAFB] gap-12 md:px-16 lg:px-16 px-4 pt-8">
        <div className="flex md:flex-row flex-col w-full justify-between">
          <span className='md:text-xl'><span className="text-purple-700">Edit </span> - {blog.title}</span>
        </div>
        <div className="flex md:flex-row flex-col w-full justify-between">
          <span className='md:text-xl'><span className="text-purple-700">Created by </span> - {blog.createdBy?.name}</span>
          <span className='md:text-xl'><span className="text-purple-700">Created on </span> - {moment(blog.createdAt).format('MMMM Do YYYY, h:mm a')}</span>
        </div>
        <div className="flex md:flex-row flex-col w-full justify-between">
        ...
        <div className="flex justify-end p-4 rounded-lg md:mb-0 mb-32">
          <button
